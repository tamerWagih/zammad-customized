class App.Ticket extends App.Model
  ...
  @configure_attributes = [
      ...
      { name: 'cc_user_ids',              display: __('CC'),           tag: 'select',   multiple: true, limit: 100, null: true, relation: 'User', width: '12%', edit: true, filter: (users) -> users.filter((user) -> user.permissions?('ticket.agent') || user.permissions?('ticket.customer')), screen: { create_middle: { shown: true } } },
    ]
