class App.Ticket extends App.Model
  ...
  @configure_attributes = [
      ...
      { name: 'cc_user_ids',              display: __('CC'),           tag: 'select', multiple: true, limit: 100, null: true, relation: 'User', edit: true, permissions: ['ticket.agent', 'ticket.customer'], filter: (users) ->
        users.filter (user) ->
