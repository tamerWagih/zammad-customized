...
class AddApprovalTicketStates < ActiveRecord::Migration[7.2]
      def up
        ...
        execute <<-SQL
          INSERT INTO ticket_state_types (id, name, created_by_id, updated_by_id, created_at, updated_at) 
          VALUES (7, 'approved', 1, 1, NOW(), NOW())
          ON CONFLICT (id) DO NOTHING;
        SQL
        
        execute <<-SQL
          INSERT INTO ticket_state_types (id, name, created_by_id, updated_by_id, created_at, updated_at) 
          VALUES (8, 'rejected', 1, 1, NOW(), NOW())
          ON CONFLICT (id) DO NOTHING;
        SQL
        ...
        execute <<-SQL
          ...
          VALUES (7, 'approved', 7, true, 1, 1, NOW(), NOW())
          ON CONFLICT (id) DO NOTHING;
        SQL
        
        execute <<-SQL
          INSERT INTO ticket_states (id, name, state_type_id, ignore_escalation, created_by_id, updated_by_id, created_at, updated_at)
          VALUES (8, 'rejected', 8, true, 1, 1, NOW(), NOW())
          ON CONFLICT (id) DO NOTHING;
        SQL
      end
